 ...
 * RWC Base.
 *
 * This file adds the required WooCommerce setup functions to the RWC Base Theme.
 *
 * @package RWC Base
...
add_action( 'wp_enqueue_scripts', 'rwc_base_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function rwc_base_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'rwc_base_woocommerce_breakpoint' );
/**
 ...
 */
function rwc_base_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'rwc_base_default_products_per_page' );
/**
 ...
 */
function rwc_base_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'rwc_base_woocommerce_pagination' );
/**
 ...
 */
function rwc_base_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'rwc-base' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'rwc-base' ) );

	return $args;

}

add_action( 'after_switch_theme', 'rwc_base_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function rwc_base_woocommerce_image_dimensions_after_theme_setup() {
	...
	rwc_base_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'rwc_base_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function rwc_base_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function rwc_base_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'rwc_base_gallery_image_thumbnail' );
/**
 ...
 */
function rwc_base_gallery_image_thumbnail( $size ) {
